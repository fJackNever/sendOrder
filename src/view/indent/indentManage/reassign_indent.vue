      ...
      <Card shadow style="margin-top:30px;">
        <Table border :columns="order_columns" :data="order_data">
            ...
            <template slot-scope="{ row, index }" slot="action">
                <Button type="primary" style="margin-right: 5px" @click="reassign_indent(index+1,'reassign')">改派</Button>
            </template>
        </Table>
      </Card>
  </div>
...
export default {
  ...
  data () {
    return {
      ...
      indentNum:'',
      indentHostData: [
        { title:'待改派订单',colSpan:3,value:60,em:true},
        { title:'已改派订单',colSpan:3,value:8,em:false},
      ],
      order_columns: [
            {
                title: '城市',
  ...
  methods: {
    ...
    reassign_indent(index,type){
        this.$router.push({path:'new_edit_indent',query:{id:index,type}});
    },
  },
  mounted () {
